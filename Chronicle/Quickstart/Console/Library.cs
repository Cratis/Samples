    ...
    void PerformCommand(object? sender, ListViewItemEventArgs e)
    {
        ...
        switch (item.Command)
        {
            ...
            case MainMenuCommand.BorrowBook:
                Application.Run(new UserAndBookSelector(async (user, book) => await Globals.EventStore.EventLog.Append(book.Id, new BookBorrowed(user.Id))));
                break;
            case MainMenuCommand.ReturnBook:
                Application.Run(new Dialogs.BorrowedBooks(async book => await Globals.EventStore.EventLog.Append(book.Id, new BookReturned())));
                break;
            case MainMenuCommand.SetBookAsOverdue:
                Application.Run(new Dialogs.BorrowedBooks(async book => await Globals.EventStore.EventLog.Append(book.Id, new BookOverdue())));
                break;
            case MainMenuCommand.ReserveBook:
                Application.Run(new UserAndBookSelector(async (user, book) => await Globals.EventStore.EventLog.Append(book.Id, new BookReservationPlaced(user.Id))));
                break;
            case MainMenuCommand.CancelReservation:
                Application.Run(new Dialogs.ReservedBooks(async book => await Globals.EventStore.EventLog.Append(book.Id, new BookReservationCancelled())));
                break;

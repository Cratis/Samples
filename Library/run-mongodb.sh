#!/usr/bin/env bash
# Run the Library Composition with MongoDB (default).
...
if [[ "${1:-}" == "--docker" ]]; then
    ...
    docker compose --profile mongodb -f "$SCRIPT_DIR/Composition/docker-compose.yml" up -d
    ...
    DATABASE_TYPE=mongodb dotnet run --project "$SCRIPT_DIR/Composition/Composition.csproj"
fi
